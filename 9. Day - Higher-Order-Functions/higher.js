...
const sum = products.reduce((acc, curr) => {
  const price = Number(curr.price); // convert price to number
  return !isNaN(price) && price > 0 ? acc + price : acc; // add only valide positive prices
}, 0);
console.log(sum);
...
// which returns the last ten countries in the countries array.

//? 7-Find out which letter is used many times as initial
// for a country name from the countries array (eg. Finland, Fiji, France etc)
